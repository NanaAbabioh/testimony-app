...
export const useAuth = () => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      try {
        if (firebaseUser) {
          ...
          // Update auth state
          const newAuthState = firebaseUser.isAnonymous ? 'anonymous' : 'authenticated';
          ...
          // Handle pending sync after successful authentication
          if (!firebaseUser.isAnonymous && localStorage.getItem('pendingSync') === 'true') {
            console.log('Triggering automatic sync after authentication...');
            await handlePostAuthSync(firebaseUser);
          }
        } else {
  ...
  /**
   * Handle sync after successful authentication
   */
  const handlePostAuthSync = async (firebaseUser) => {
    try {
      console.log('Starting post-authentication sync...');

      // Get saved clips from localStorage
      const savedClips = JSON.parse(localStorage.getItem('saved_clips') || '[]');
      console.log(`Found ${savedClips.length} saved clips to sync`);

      if (savedClips.length === 0) {
        console.log('No clips to sync');
        localStorage.removeItem('pendingSync');
        localStorage.removeItem('syncEmail');
        alert('Account Setup Complete\n\nYour account has been successfully linked. You can now save testimonies and they will automatically sync across all your devices.');
        return;
      }
      ...
      // Sync clips to Firestore
      const response = await fetch('/api/sync-clips', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        // Clean up pending sync flags
        localStorage.removeItem('pendingSync');
        localStorage.removeItem('syncEmail');

        alert(`Account Setup Complete\n\nSuccessfully synced ${savedClips.length} saved testimonies to the cloud.\n\nYour testimonies are now available across all your devices. Any new testimonies you save will automatically sync to the cloud.`);
      } else {
        console.error('Sync failed:', result);
        alert(`Sync Failed\n\n${result.error}\n\nYour testimonies are still saved locally on this device. You can try the sync process again later from the menu.`);
      }

    } catch (error) {
      console.error('Error during post-auth sync:', error);
      alert(`Sync Error\n\n${error.message}\n\nYour testimonies are still saved locally on this device. You can try the sync process again later from the menu.`);
    }
  };
