...
import { Clip, ClipDTO } from "../../../../lib/types";
...
interface MostSavedCursor {
  savedCount: number;
  createdAtMs: number;
}

type CursorData = RecentCursor | MostSavedCursor;
 ...
 */
function encodeCursor(obj: CursorData): string {
  try {
 ...
 */
function decodeCursor(s?: string | null): CursorData | null {
  if (!s) return null;
...
function validateQueryParams(searchParams: URLSearchParams) {
  ...
  const monthValue = searchParams.get("month")?.trim() || undefined;
  const yearValue = searchParams.get("year")?.trim() || undefined;
  const episodeValue = searchParams.get("episode")?.trim() || undefined;
  ...
  const cursor = decodeCursor(searchParams.get("cursor")?.trim());
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const { categoryId, month, episode, sort, limit, cursor } = validateQueryParams(searchParams);
    ...
    let items: ClipDTO[] = snapshot.docs
      .map((doc, index) => {
        const data = doc.data() as any; // Use any to access all fields
    ...
    if (categoryId) {
      ...
      if (items.length === limit && snapshot.docs.length > 0) {
        ...
        const lastData = lastDoc.data() as Clip;
        const createdAtMs = (lastData.createdAt as Timestamp)?.toMillis?.() || Date.now();
    ...
    // - s-maxage=120: CDN can cache for 2 minutes
    response.headers.set(
      "Cache-Control", 
      "public, max-age=60, stale-while-revalidate=300, s-maxage=120"
    );
    
    // Add ETag for better cache validation
    const etag = `"clips-${sort}-${categoryId || 'all'}-${month || 'all'}-${episode || 'all'}-${items.length}-${queryTime}"`;
